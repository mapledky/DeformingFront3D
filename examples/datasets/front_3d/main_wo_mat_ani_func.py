...
def render_scenes_with_animations(front_path,
    ...
    if not sample_shapenet_obj(loaded_objects,shapenet_json , shapenet_folder, sample_number=shapenet_number):
        return
    furniture_manager = FurnitureManage(loaded_objects)
    ...
    proximity_checks = {"min": 2.8, "avg": {"min": 2.8, "max": 10}, "no_background": True}
    ...
    for i in range(12000):
        if i % 1000 == 0 and i != 0:
            print('check 1000')
            proximity_checks = {"min": max(2.8 - 0.2 * i / 1000, 1.6), "avg": {"min": 2.8, "max": 10}, "no_background": True}
        ...
        if poses >= anime_number or poses >= 6:
            break
    ...
    for index, anime_file in enumerate(anime_files):
        ...
        for frame in range(0, num_frames, anim_flow_skip):
            ...
            point_cloud_target, back_indices= depth2pointcloud(data.get('depth'), cam_info, indices=back_indices)
